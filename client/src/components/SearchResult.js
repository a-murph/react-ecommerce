...
const SearchResult = props => (
	<Link className="search-result-card" to={{ pathname: `/product/${props.product._id}`, state: { product: props.product} }}>
		...
		<div className="search-result-card-text">
			...
			<div><span className="search-body-font">${props.product.price.toFixed(2)}</span> <i class="fas fa-check-circle icon"></i> <span className="search-body-font">FREE SHIPPING</span></div>
