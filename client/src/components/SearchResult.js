	...
	return (
		<Link className="search-result-card" to={{ pathname: `/product/${props.product._id}`, state: { product: props.product} }}>
			...
			<div className="search-result-card-text">
				...
				<div> <img className="review-stars" alt="" src={`/img/stars/${props.product.rating}-star.svg`}/> <span className="search-body-font">{props.product.rating} Stars </span><span className="search-body-font">({props.product.reviews.length} Reviews)</span></div>
			</div>
		</Link>
	);
};
